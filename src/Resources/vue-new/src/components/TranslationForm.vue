            ...
            <div class="col-sm-6">
                <div class="form-group">
                    ...
                    <input type="text" id="lxk_trans_unit_key" class="form-control" v-model="key" v-validate="{ required: true }"
                           data-vv-name="key">
                    <p v-for="(error, errorIndex) in errors.collect('key')" v-show="errors.has('key')" :key="errorIndex" class="text-danger">{{ error }}</p>
                </div>
                ...
                <div class="form-group">
                    ...
                    <select id="lxk_trans_unit_domain" class="form-control" v-model="domain" v-validate="{ required: true }"
                            data-vv-name="domain">
                        <option :value="domain" v-for="domain in domains" :key="domain">{{ domain }}</option>
                    </select>
                    <p v-for="(error, errorIndex) in errors.collect('domain')" v-show="errors.has('domain')" :key="errorIndex" class="text-danger">{{ error }}</p>
                ...
                <div class="form-group">
                    <div v-for="(translation, index) in translations" :key="index">
                        ...
                        <template v-if="translation.locale == 'en'">
                            ...
                            <p v-for="(error, errorIndex) in errors.collect('defaultLocale')" v-show="errors.has('defaultLocale')" :key="errorIndex" class="text-danger">
                                {{ error }}
                            </p>
                        </template>
                        <template v-else>
